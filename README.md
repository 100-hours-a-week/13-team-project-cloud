...
| Next.js | 500MB~1GB | 5~10% | SSR 혼합, 빌드 시 일시적 피크 |
| Spring Boot | 1.5~2GB | 10~20% | 힙 1~1.5GB + 메타스페이스 |
| PostgreSQL | 2~3GB | 5~15% | shared_buffers 1GB + OS 캐시 |
| Qdrant | 1~2GB | 5~10% | 벡터 10만 개 이하 기준 |
| OS/Docker | 1~1.5GB | 5% | 시스템 프로세스, 로깅 |
| **합계** | **~8GB** | **~50%** | 여유 8GB 확보 |

**선정 근거:**

1. **RAM 16GB 충분**
   - 예상 사용량 ~8GB, 여유분 ~8GB 확보
   - 여유 RAM은 OS 페이지 캐시로 활용 → DB 성능 향상
...
3. **Graviton2(ARM) 가성비**
   ...
   - Node.js, Java, PostgreSQL, Qdrant 모두 ARM 호환
...
|--------|------|
| A | 단일 EC2 (현재 선택) |
| B | 서비스 분리 (Public Subnet) - EC2 3대 + RDS |
| C | 서비스 분리 + Private Subnet - EC2 3대 + RDS + ALB + NAT |
...
| EC2 Qdrant | - | t4g.small ~$12 | t4g.small ~$12 |
| RDS PostgreSQL | - | db.t4g.micro ~$15 | db.t4g.micro ~$15 |
| EC2 (통합) | t4g.xlarge ~$98 | - | - |
| EBS | gp3 100GB ~$11 | gp3 20GB x3 ~$7 | gp3 20GB x3 ~$7 |
| ALB | - | - | ~$18 |
| NAT Gateway | - | - | ~$35 |
| 데이터 전송 | ~$1 | ~$3 | ~$5 |
| **월 총계** | **~$110** | **~$61** | **~$116** |
...
- Option B가 더 저렴하나, 서비스 4개 관리 오버헤드 발생
- 트래픽 증가 시 Option B/C로 전환 고려

###### 스케일업 기준

다음 상황 발생 시 m7g.xlarge($143/월)로 업그레이드 검토:
- [ ] CPU 크레딧 잔량 지속 감소
- [ ] 메모리 사용률 80% 이상 지속
- [ ] DAU 100명 이상

<!-- ###### GPU 서버 비용 (RunPod - Qwen 7B vLLM)
...
| 모델 | Qwen 7B |
| 추론 엔진 | vLLM |
| 필요 VRAM | ~14-16GB (FP16) |
| 권장 GPU | RTX 3090/4090 (24GB) 또는 L4 (24GB) |
| 용도 | 영수증 OCR 등 |

**RunPod 가격 옵션 (Pod - 24/7 운영 시):**

| 방식 | GPU | 시간당 가격 | 월간 비용 (730h) |
|------|-----|------------|-----------------|
| Community Cloud | RTX 3090 (24GB) | $0.22 | ~$161 |
| Community Cloud | RTX 4090 (24GB) | $0.34 | ~$248 |
| Community Cloud | L4 (24GB) | $0.44 | ~$321 |
| Secure Cloud | RTX 3090 (24GB) | $0.46 | ~$336 |
| Secure Cloud | RTX 4090 (24GB) | $0.59 | ~$431 |
| Secure Cloud | L4 (24GB) | $0.39 | ~$285 |

**RunPod Serverless 가격 (24GB GPU 기준):**

| 타입 | 초당 가격 | 시간당 환산 |
|------|----------|------------|
| Flex Worker | $0.00019 | ~$0.68 |
| Active Worker | $0.00013 | ~$0.47 |

**DAU 30 기준 Serverless 예상 비용:**

| 항목 | 값 |
|------|-----|
| 일일 OCR 요청 | ~50건 (DAU 30 × 1-2건/user) |
| 요청당 처리 시간 | ~3초 |
| 월간 요청 | ~1,500건 |
| 월간 GPU 시간 | ~1.25시간 (4,500초) |
| **월간 비용 (Flex)** | **~$0.9** |

**권장:** Serverless Flex 방식 (사용하지 않을 때 비용 $0, 실사용량만 과금) -->
...
- 단일 EC2 인스턴스에 Spring(API) + Front + DB + Qdrant를 함께 배포
...
- 서비스가 복잡해질 때(WebSocket, GPU 워크로드 등) 커스텀 가능
...
| 문제 | 설명 |
|------|------|
| 단일 장애 지점 | EC2 한 대 장애 시 API/Front/DB/Qdrant 동시 다운 |
| 연쇄 장애 | DB I/O 상승 → API 지연 → Qdrant 타임아웃 → 전체 장애 |
...
| 문제 | 설명 |
|------|------|
| CPU/메모리/디스크/IO 경쟁 | DB(디스크) + Qdrant(메모리) + Spring(CPU) + Front 빌드가 한 머신에서 경쟁 |
| Qdrant 성능 문제 | 메모리 부족 시 HNSW 인덱스 로딩 지연 → 검색 성능 저하 |
...
| 문제 | 설명 |
|------|------|
| 수평 확장 불가 | DB/Qdrant가 로컬이면 API 인스턴스 증설 불가 (데이터가 묶임) |
