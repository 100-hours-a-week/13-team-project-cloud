...
| 관련 문서 | [K8S-001 Kubernetes 최종 설계서](../kubernetes/K8S-001-final-design.md), [K8S-006 Service 데이터플레인 비교 연구](../kubernetes/K8S-006-service-dataplane-comparison-study.md), [K8S-007 iptables 심화](../kubernetes/K8S-007-kube-proxy-iptables-deep-dive.md), [K8S-008 nftables 심화](../kubernetes/K8S-008-kube-proxy-nftables-deep-dive.md), [K8S-009 ipvs 심화](../kubernetes/K8S-009-kube-proxy-ipvs-deep-dive.md), [K8S-010 eBPF replacement 메모](../kubernetes/K8S-010-ebpf-service-dataplane-note.md) |
...
또한 `ipvs`는 현재 Kubernetes 기준으로 deprecated 방향이므로 채택하지 않고, `nftables`는 후속 재검토 후보로 둔다. eBPF 기반 `kube-proxy replacement`는 현재 단계에서 제외한다.
...
### 3.3. 이후 전환 경로를 남길 수 있는가

지금 `iptables`를 기준선으로 두더라도, 이후 `nftables`나 eBPF replacement로 넘어갈 여지는 열려 있어야 한다.
...
### 5.3. `nftables`는 유망하지만, 지금 당장 기준선으로 올리기에는 운영 전제가 더 필요하다

Kubernetes는 `nftables`를 stable이며 장기적으로 더 나은 구현이라고 설명한다. 다만 현재는 Linux 기본 모드가 아니고, kernel 5.13+와 관련 운영 이해도까지 함께 요구한다.

현재 단계에서는 이 전제를 즉시 기준선으로 올리기보다, 후속 재검토 후보로 두는 편이 더 적절하다.

### 5.4. `ipvs`는 deprecated 방향이므로 채택하지 않는다
...
### 5.5. eBPF replacement는 현재 단계에서 제외한다
...
유망한 후속 후보지만, 현재 단계에서 운영 이해도와 환경 전제가 더 필요하다.

### 6.2. `ipvs`

deprecated 방향이며, 현재 Kubernetes가 권장하는 기본 경로가 아니다.

### 6.3. eBPF replacement
...
즉 현재 구조에서는 `Service ClusterIP` 전달 계층을 `kube-proxy iptables`로 운영하고, 나머지 대안은 비교 연구와 후속 확장 경로로 관리한다.
...
- 서비스 수와 endpoint 수가 커지면 `nftables` 재검토 필요성이 커질 수 있다.
...
1. Linux kernel과 운영 환경이 준비되면 `nftables` 전환 재검토
2. `Service` 수와 endpoint 수 증가 시 `iptables` rule 규모 영향 점검
3. eBPF replacement의 실익이 커질 경우 별도 재평가

---

## 10) 최종 판단 문장

> 본 설계에서는 Kubernetes `Service` 데이터플레인을 `kube-proxy`로 유지하고, 초기 모드는 `iptables`를 기준선으로 채택한다. 이는 다른 모드가 낯설어서가 아니라, 현재 단계에서 `Service ClusterIP` 전달 계층을 가장 보수적이고 설명 가능한 기본 경로로 먼저 고정하기 위한 판단이다. `nftables`는 유망한 후속 후보로 두고, `ipvs`는 deprecated 방향이므로 채택하지 않으며, eBPF 기반 replacement는 현재 단계에서 제외한다.
