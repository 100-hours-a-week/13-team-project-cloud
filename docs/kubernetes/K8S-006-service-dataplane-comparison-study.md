# K8S-006: Service 데이터플레인 비교 연구 - kube-proxy iptables vs nftables vs ipvs vs eBPF replacement
...
| 관련 문서 | [K8S-001 Kubernetes 최종 설계서](K8S-001-final-design.md), [DR-010 Service 데이터플레인 전략 선정](../architecture/DR-010-kube-proxy-and-service-dataplane-strategy.md), [K8S-007 iptables 심화](K8S-007-kube-proxy-iptables-deep-dive.md), [K8S-008 nftables 심화](K8S-008-kube-proxy-nftables-deep-dive.md), [K8S-009 ipvs 심화](K8S-009-kube-proxy-ipvs-deep-dive.md), [K8S-010 eBPF replacement 메모](K8S-010-ebpf-service-dataplane-note.md) |
...
4. 이후 `nftables`나 eBPF replacement로 넘어갈 여지를 남기는가
...
| `nftables` | `Service IP + protocol + port`를 `verdict map`으로 조회해 Service 체인으로 분기 | 첫 패킷 latency와 rule update 효율이 더 좋음 | kernel 5.13+, `nft` 도구, NodePort/방화벽/conntrack 차이 이해 필요 | 후속 재검토 1순위 |
...
## 10) `nftables`를 어떻게 봤는가

Kubernetes 공식 문서와 블로그 기준으로 `nftables` 모드는 stable이며, 장기적으로는 `iptables`와 `ipvs`를 대체하는 방향으로 설명된다.
...
즉 `nftables`는 유망한 후속 후보지만, 현재 단계에서 바로 기준선으로 올리기에는 운영 이해도와 환경 검토가 더 필요하다.

상세 내용은 [K8S-008 nftables 심화](K8S-008-kube-proxy-nftables-deep-dive.md)에서 다룬다.
...
하지만 Kubernetes 공식 문서는 현재 `ipvs` 모드를 deprecated로 표시한다. 이유는 IPVS API가 Kubernetes Service API와 잘 맞지 않아 edge case를 완전히 구현하지 못했고, `nftables`가 성능과 구현 정합성 면에서 더 나은 대안으로 자리 잡고 있기 때문이다.
